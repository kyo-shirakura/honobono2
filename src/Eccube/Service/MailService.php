...
class MailService
{
     ...
     * Send Payslip mail.
     *
     * @param $Payslip 給与明細メール
     */
    public function sendPayslipMail(\Customize\Entity\Payslip $Payslip, \Customize\Entity\Staff $Staff)
    {
        log_info('給与明細メールsendPayslipMail開始');

        if( $Staff->getEmail() !== null && $Staff->getEmail() != '' ){
            $send_to = $Staff->getEmail();
            log_info('---To: '. $send_to);

            // 顧客用テンプレ
            $MailTemplate = $this->mailTemplateRepository->findOneBy([
                'file_name' => 'Mail/payslip_mail.twig'
            ]);
            ...
            $message = (new Email())
                ...
                ->to($this->convertRFCViolatingEmail( $send_to ))
            ...
            // 添付ファイル
            if( $Payslip->getPdfFile() ){
                $message->attachFromPath(
                    $this->eccubeConfig["eccube_save_image_dir"]. '/'. $Payslip->getPdfFile(),
                );
            }
        ...
        log_info('給与明細メールsendPayslipMail完了');
    }

    /**
     * Send Invoice mail.
     *
     * @param $Invoice 請求書メール
     */
    public function sendInvoiceMail(\Customize\Entity\Invoice $Invoice, \Eccube\Entity\Customer $Customer)
    {
        log_info('給与明細メールsendInvoiceMail開始');

        if( $Customer->getEmail() !== null && $Customer->getEmail() != '' ){
            $send_to = $Customer->getEmail();
            log_info('---To: '. $send_to);

            // 顧客用テンプレ
            $MailTemplate = $this->mailTemplateRepository->findOneBy([
                'file_name' => 'Mail/invoice_mail.twig'
            ]);
            $body = $this->twig->render($MailTemplate->getFileName(), [
                'Invoice' => $Invoice,
                'Customer' => $Customer,
            ...
            if( $Invoice->getPdfFile() ){
                $message->attachFromPath(
                    $this->eccubeConfig["eccube_save_image_dir"]. '/'. $Invoice->getPdfFile(),
                );
            }
        ...
        log_info('給与明細メールsendInvoiceMail完了');
    }

    /**
     * Send StaffReport complete mail.
     *
     * @param $StaffReport 業務報告メール
     */
    public function sendStaffReportCompleteMail(\Customize\Entity\Report $Report, Customer $Customer, \Customize\Entity\Staff $Staff, $work_details, $safety_confirmation)
    {
        log_info('業務報告メールsendStaffReportCompleteMail開始');
//        echo '<br>業務報告メールsendStaffReportCompleteMail開始';
        if( $Customer->getEmail() === null || $Customer->getEmail() == '' ){
            $this->sendReport( $Report, $Customer, $Staff, $work_details, $safety_confirmation, 'info@honobono-vision.com', 'Mail/business_report_customer_mail.twig' );
        }else{
            $this->sendReport( $Report, $Customer, $Staff, $work_details, $safety_confirmation, $Customer->getEmail(), 'Mail/business_report_customer_mail.twig' );
        }
        if( $Staff->getEmail() !== null && $Staff->getEmail() != '' ){
            $this->sendReport( $Report, $Customer, $Staff, $work_details, $safety_confirmation, $Staff->getEmail(), 'Mail/business_report_staff_mail.twig' );
        }
        $this->sendReport( $Report, $Customer, $Staff, $work_details, $safety_confirmation, $this->BaseInfo->getEmail01(), 'Mail/business_report_office_mail.twig' );
//        echo '<br>業務報告メールsendStaffReportCompleteMail完了';

        log_info('業務報告メールsendStaffReportCompleteMail完了');
    }

    public function sendReport(\Customize\Entity\Report $Report, Customer $Customer, \Customize\Entity\Staff $Staff, $work_details, $safety_confirmation, $send_to, $mailTemplateName )
    {
//        echo '<br>send_to: '. $send_to;
        // 顧客用テンプレ
        $MailTemplate = $this->mailTemplateRepository->findOneBy([
            'file_name' => $mailTemplateName
        ]);
        $body = $this->twig->render($MailTemplate->getFileName(), [
            'Report' => $Report,
            'Customer' => $Customer,
    ...
    //                'activateUrl' => $activateUrl,
            ...
            // 添付ファイル
            if( $Report->getPaymentReceipt() ){
                $message->attachFromPath(
                    $this->eccubeConfig["eccube_save_image_dir"]. '/'. $Report->getPaymentReceipt(),
                );
            }

            // 顧客用送信
            $this->mailer->send($message);
            log_info('業務報告メール送信完了', ['id' => $Report->getId()]);
        }else{
            log_info('業務報告メール送信先未設定', ['id' => $Report->getId()]);
        }
    }
